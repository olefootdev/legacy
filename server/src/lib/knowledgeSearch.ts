...
export function searchKnowledge(
  ...
  for (const entry of knowledgeBase) {
    ...
    if (entry.category === 'docs') score *= 1.5;
    else if (entry.category === 'code') score *= 1.2;
...
export function generateAnswer(
  ...
  if (topResult.score > 10) confidence = 'high';
  else if (topResult.score > 5) confidence = 'medium';
 ...
 */
export const FAQ_ANSWERS: Record<string, { answer: string; sources: string[] }> = {
  'como ganhar exp': {
 ...
 */
export function findFAQAnswer(query: string): { answer: string; sources: string[] } | null {
