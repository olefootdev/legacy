...
function bearerFromAuthHeader(h: string | undefined): string | null {
...
function uploadTokenBypassOk(serverSecret: string | undefined, clientHeader: string | undefined): boolean {
  const s = serverSecret?.trim();
  const c = clientHeader?.trim();
  if (!s || !c || s.length < 16) return false;
  return s === c;
}
...
pinataMediaRoutes.post('/api/media/pinata/upload', async (c) => {
  ...
  if (!pinataJwt) {
    return c.json<PinataUploadApiResponse>(
      { ok: false, error: 'PINATA_JWT não configurado no servidor.', uploadStatus: 'error' },
      503,
    );
  }
  ...
  if (!usedBypass) {
      ...
      return c.json<PinataUploadApiResponse>(
        {
          ok: false,
          error:
            'SUPABASE_URL + SUPABASE_ANON_KEY em falta no servidor, ou usa OLEFOOT_PINATA_UPLOAD_TOKEN + header X-Olefoot-Pinata-Upload-Token (dev).',
          uploadStatus: 'error',
        },
        503,
      );
      ...
      return c.json<PinataUploadApiResponse>(
        {
          ok: false,
          error:
            'Sessão Supabase em falta (faz login no app) ou define OLEFOOT_PINATA_UPLOAD_TOKEN no servidor e VITE_OLEFOOT_PINATA_UPLOAD_TOKEN no cliente para o Admin local.',
          uploadStatus: 'error',
        },
        401,
      );
    ...
    if (authErr || !userData?.user) {
      ...
      return c.json<PinataUploadApiResponse>(
        { ok: false, error: 'Sessão inválida ou expirada.', uploadStatus: 'error' },
        401,
      );
    }
  } else {
  ...
  try {
    ...
    return c.json<PinataUploadApiResponse>(
      { ok: false, error: 'Corpo multipart inválido.', uploadStatus: 'error' },
      400,
    );
  }
  ...
  if (!file || typeof file === 'string' || !(file instanceof File)) {
    return c.json<PinataUploadApiResponse>(
      { ok: false, error: 'Campo "file" obrigatório (ficheiro).', uploadStatus: 'error' },
      400,
    );
  }
  ...
  if (!up.ok) {
    const st = up.status;
    const status =
      st === 400 || st === 401 || st === 403 || st === 413 || st === 502 || st === 503 ? st : 502;
    return c.json<PinataUploadApiResponse>(
      { ok: false, error: up.message, uploadStatus: 'error' },
      status,
    );
  }

  const media: HostedMediaDescriptor = {
    provider: 'pinata',
    cid: up.data.cid,
  ...
  return c.json<PinataUploadApiResponse>({ ok: true, media });
});
