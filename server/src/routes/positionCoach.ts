...
const POS_SYSTEM_PROMPTS: Record<string, string> = {
...
positionCoachRoutes.post('/api/position-coach/train', rateLimit(20), async (c) => {
  ...
  let body: TrainBody;
  try {
    body = (await c.req.json()) as TrainBody;
  } catch {
  ...
  const recentStr =
    Array.isArray(body.recentResults) && body.recentResults.length > 0
...
Com base neste tópico, no perfil do jogador e no conhecimento tático registado acima, define os pesos de ação e traits atualizados para este jogador. Responde só com o JSON pedido.`;
  ...
  if (!r.ok) {
    return c.json(
      { ok: false, error: r.error ?? 'Falha Anthropic.', rawAssistant: r.rawAssistant },
      502,
    );
  }
