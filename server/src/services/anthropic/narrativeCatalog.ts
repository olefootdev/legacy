...
const SYSTEM_PROMPT = jsonSystemPrompt(
  `Você é o "GameSpirit", narrador de futebol do OLEFOOT. Gera linhas curtas
...
estádio"]} — o motor escolhe uma ao renderizar.`,
  `{ templates: [{ template: string, variables?: { [key: string]: string[] } }] }`,
);

function userPromptFor(slot: CatalogSlot): string {
...
interface RawResponse {
  templates: { template: string; variables?: Record<string, string[]> }[];
}

export async function generateSlot(slot: CatalogSlot): Promise<GeneratedTemplate[]> {
  const r = await callAnthropic<RawResponse>({
    model: 'haiku',
  ...
  if (!r.ok || !r.json?.templates) {
    throw new Error(
      `Falha a gerar slot ${slot.category}/${slot.intensity}: ${r.error ?? 'resposta vazia'}`,
    );
  }
 ...
 */
export async function generateCatalog(slots: CatalogSlot[]): Promise<GeneratedTemplate[]> {
  ...
  for (let i = 0; i < slots.length; i++) {
    ...
    console.log(
      `  [${i + 1}/${slots.length}] ${slot.category}/${slot.intensity}/${slot.personaVibe} → ${got.length} templates`,
    );
  }
  return all;
}
