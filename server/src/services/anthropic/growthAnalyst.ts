...
const GROWTH_SYSTEM = jsonSystemPrompt(
  `Você é o **Analista de Growth** da OLEFOOT — jogo/fintech de gestão de
...
- Português brasileiro.`,
  `{ daily_review: string 2-4 frases,
...
export async function runGrowthAnalyst(input: {
  snapshot: unknown;
  founderNote?: string;
}): Promise<{ ok: boolean; analysis?: GrowthAnalystResult; error?: string; rawAssistant?: string }> {
  ...
  const r = await callAnthropic<GrowthAnalystResult>({
    model: 'sonnet',
