...
type PinataApiEnvelope = { data?: PinataApiData; error?: { message?: string } };

function gatewayUrlForCid(cid: string, gatewayPrefix: string): string {
...
export async function uploadBufferToPinata(input: {
  ...
  | { ok: true; data: PinataUploadOk; publicUrl: string }
...
> {
  ...
  let json: PinataApiEnvelope | null = null;
  try {
    json = JSON.parse(rawText) as PinataApiEnvelope;
  } catch {
    json = null;
  }
  ...
  const sizeBytes =
    typeof row.size === 'number' && Number.isFinite(row.size) ? Math.round(row.size) : buffer.byteLength;
  const createdAt =
