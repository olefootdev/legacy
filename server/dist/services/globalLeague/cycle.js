...
function nextKickoff(fromMs) {
    return Math.ceil((fromMs + 1000) / ROUND_INTERVAL_MS) * ROUND_INTERVAL_MS;
}
function generatePlayoffFixtures(teams, seasonId, nowMs) {
    const rounds = [];
    const fixtures = [];
    ...
    for (let rn = 1; rn <= 6; rn++) {
        ...
        rounds.push({ id: roundId, season_id: seasonId, round_number: rn,
            round_type: 'playoff', phase, is_returning: isReturn, status: 'scheduled', scheduled_kickoff_ms: kickoffMs });
...
function distributeIntoDivisions(teams) {
    const sorted = [...teams].sort((a, b) => {
        if (b.playoff_points !== a.playoff_points)
            return b.playoff_points - a.playoff_points;
        if (b.playoff_wins !== a.playoff_wins)
            return b.playoff_wins - a.playoff_wins;
...
function generateLeagueFixtures(teams, seasonId, nowMs) {
    const rounds = [];
    const fixtures = [];
    const byDiv = new Map();
    ...
    for (let rn = 1; rn <= totalRounds; rn++) {
        ...
        rounds.push({ id: roundId, season_id: seasonId, round_number: rn,
            round_type: 'league', phase: null, is_returning: isReturn, status: 'scheduled', scheduled_kickoff_ms: kickoffMs });
...
export async function runGlobalLeagueCycle(sb) {
    const now = Date.now();
    ...
    if (state.status === 'waiting_teams') {
        ...
        if (rounds.length === 0)
            return { ok: true, step: 'auto-start', skipped: true, reason: 'gen-empty' };
        await sb.from('global_league_rounds').upsert(rounds, { onConflict: 'id' });
        if (fixtures.length > 0)
            await sb.from('global_league_fixtures').upsert(fixtures, { onConflict: 'id' });
        await sb.from('global_league_state')
            .update({ status: 'playoffs', current_playoff_round: 1 }).eq('id', 'current');
    ...
    if (state.status === 'playoffs') {
        ...
        const allDone = (pRounds ?? []).length > 0 && (pRounds ?? []).every((r) => r.status === 'finished');
        if (allDone) {
            const { data: teamsData } = await sb.from('global_league_teams').select('*');
            const teams = teamsData ?? [];
            ...
            await sb.from('global_league_teams').upsert(distributed, { onConflict: 'id' });
            if (rounds.length > 0)
                await sb.from('global_league_rounds').upsert(rounds, { onConflict: 'id' });
            if (fixtures.length > 0)
                await sb.from('global_league_fixtures').upsert(fixtures, { onConflict: 'id' });
    ...
    for (const t of preTeams ?? [])
        teamById.set(t.id, t);
    const fixturesUpdated = [];
    const eventsToInsert = [];
    const teamDelta = new Map();
    const newInjuries = new Map();
    const yellowsThisRound = new Map();
    for (const fx of fixturesData) {
        ...
        ha.gf += sim.score_home;
        ha.ga += sim.score_away;
        teamDelta.set(fx.home_team_id, ha);
        const aa = teamDelta.get(fx.away_team_id) ?? { gf: 0, ga: 0 };
        aa.gf += sim.score_away;
        aa.ga += sim.score_home;
        teamDelta.set(fx.away_team_id, aa);
        if (sim.injured_side) {
        ...
        if (sim.home_yellow)
            yellowsThisRound.set(fx.home_team_id, (yellowsThisRound.get(fx.home_team_id) ?? 0) + 1);
        if (sim.away_yellow)
            yellowsThisRound.set(fx.away_team_id, (yellowsThisRound.get(fx.away_team_id) ?? 0) + 1);
    }
    if (fixturesUpdated.length > 0) {
        const { error: fxUpErr } = await sb.from('global_league_fixtures').upsert(fixturesUpdated, { onConflict: 'id' });
        if (fxUpErr)
            console.error('[cycle] fixtures upsert failed:', fxUpErr.message);
    }
    if (eventsToInsert.length > 0) {
        const { error: evErr } = await sb.from('global_league_events').upsert(eventsToInsert, { onConflict: 'id' });
        if (evErr)
            console.error('[cycle] events upsert failed:', evErr.message);
    }
    if (teamById.size > 0) {
        ...
        const { error: teamsErr } = await sb.from('global_league_teams').upsert(updated, { onConflict: 'id' });
        if (teamsErr)
            console.error('[cycle] teams upsert failed:', teamsErr.message);
    }
...
export async function recoverStaleLiveRound(sb, now = Date.now()) {
...
export async function enrollClubInGlobalLeague(sb, opts) {
    ...
    const { error } = await sb.from('global_league_teams').upsert(team, { onConflict: 'manager_id' });
    if (error) {
        console.error('[enrollClub] failed:', error.message);
