...
function buildResponsesInput(ctx) {
...
function parseModelJson(text) {
    ...
    const confidence = typeof confRaw === 'number' && Number.isFinite(confRaw)
        ...
        : typeof confRaw === 'string' && confRaw.trim()
            ...
            : NaN;
    if (!decision || !narration || !Number.isFinite(confidence))
        return null;
    return { decision, confidence, narration };
}
function gamespiritModel() {
    return (process.env.OPENAI_GAMESPIRIT_MODEL?.trim() ||
 ...
 */
export async function getGameDecision(ctx, options = {}) {
...
//# sourceMappingURL=getGameDecision.js.map