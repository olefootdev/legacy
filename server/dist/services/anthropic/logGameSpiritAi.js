...
export function logGameSpiritAiFireAndForget(input) {
    if (process.env.GAMESPIRIT_AI_LOG_SUPABASE !== '1')
        return;
    ...
    void sb
        ...
        .then(({ error }) => {
        if (error)
            console.warn('[gamespirit] log supabase:', error.message);
    });
}
//# sourceMappingURL=logGameSpiritAi.js.map