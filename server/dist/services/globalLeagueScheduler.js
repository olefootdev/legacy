...
import { generatePlayoffRounds as logicGenPlayoffs, distributeIntoDivisions as logicDistribute, generateLeagueRounds as logicGenLeague, applyPromotionRelegation as logicPromoRele, } from './globalLeagueLogic.js';
...
function generateEvents(homeScore, awayScore, fixture) {
    const events = [];
    const usedMinutes = new Set();
    const addGoal = (side, teamName) => {
        let minute;
        do {
    ...
    for (let i = 0; i < homeScore; i++)
        addGoal('home', fixture.homeTeamName);
    for (let i = 0; i < awayScore; i++)
        addGoal('away', fixture.awayTeamName);
    return events.sort((a, b) => a.minute - b.minute);
}
/** Atualiza pontos dos times após rodada de playoff */
function applyPlayoffResults(teams, fixtures) {
...
function applyLeagueResults(teams, fixtures) {
    return teams.map(team => {
        const fixture = fixtures.find(f => f.homeTeamId === team.id || f.awayTeamId === team.id);
        if (!fixture)
            return team;
...
async function persistRound(state, round, options = {}) {
    const sb = getSupabaseAdmin();
    if (!sb)
        return;
    ...
    if (round.fixtures.length > 0) {
        await sb.from('global_league_fixtures').upsert(round.fixtures.map(f => ({
            id: f.id,
            round_id: roundId,
            division: f.division,
        ...
        if (allEvents.length > 0) {
            await sb.from('global_league_events').upsert(allEvents.map(e => ({
                id: e.id,
...
const lastPersistedMinute = new Map();
/** Processa uma rodada: scheduled → live → finished → próxima */
async function processRound(state, round, rounds, setCurrentRound, applyResults, nowMs) {
...
async function tryAutoStartPlayoffs(state, nowMs) {
    if (state.status !== 'waiting_teams')
        return false;
    if (state.teams.length < state.minTeamsRequired)
        return false;
 ...
 */
async function tryTransitionToLeague(state, nowMs) {
 ...
 */
async function tryTransitionToNewSeason(state, nowMs) {
    ...
    const promoted = logicPromoRele(state.teams.map(teamToLite), state.promotionPercentage, state.relegationPercentage);
...
async function persistTeamsAndState(state) {
    const sb = getSupabaseAdmin();
    if (!sb)
        return;
    await sb.from('global_league_state').upsert({
...
async function tick() {
    ...
    // ─── Auto-start playoffs ──────────────────────────────────────────────
    if (await tryAutoStartPlayoffs(state, nowMs))
        return;
    // ─── Playoffs ────────────────────────────────────────────────────────
...
export function startGlobalLeagueScheduler() {
    const sb = getSupabaseAdmin();
    if (!sb) {
...
//# sourceMappingURL=globalLeagueScheduler.js.map