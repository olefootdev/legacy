...
matchRoutes.post('/matches', async (c) => {
    ...
    if (!supabaseAdmin) {
        return c.json({ error: 'Supabase não configurado neste servidor (SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY em server/.env).' }, 503);
    }
    ...
    const { data: club } = await supabaseAdmin
        ...
        .maybeSingle();
    if (!club)
        return c.json({ error: 'Forbidden: home_club_id not owned by user' }, 403);
...
matchRoutes.post('/matches/:id/events', async (c) => {
    ...
    const owner = match.clubs?.owner_id;
