...
function teamFromProfile(profile, squad, existing) {
...
globalLeagueRoutes.post('/start-season', async (c) => {
    ...
    const seasonName = String(body.seasonName);
    ...
    const minTeamsRequired = Number(body.minTeamsRequired ?? 2);
...
globalLeagueRoutes.post('/backfill-teams', async (c) => {
    ...
    if (!sb)
        return c.json({ error: 'Supabase não configurado' }, 503);
    const [{ data: profiles, error: profilesErr }, { data: squads, error: squadsErr }, { data: existing, error: teamsErr },] = await Promise.all([
    ...
    if (profilesErr)
        return c.json({ error: profilesErr.message }, 500);
    if (squadsErr)
        return c.json({ error: squadsErr.message }, 500);
    if (teamsErr)
        return c.json({ error: teamsErr.message }, 500);
    const squadByUser = new Map((squads ?? []).map((s) => [s.user_id, s]));
    const existingByManager = new Map((existing ?? []).map((t) => [t.manager_id, t]));
    ...
    if (teams.length > 0) {
        const { error } = await sb.from('global_league_teams').upsert(teams, { onConflict: 'manager_id' });
        if (error)
            return c.json({ error: error.message }, 500);
    }
    return c.json({
