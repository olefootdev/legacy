...
function genesisListingPriceExp(mintOverall) {
...
marketRoutes.post('/api/market/buy', rateLimit(20), async (c) => {
    ...
    const sb = getSupabaseAdmin();
    if (!sb)
        return c.json({ ok: false, error: 'Serviço indisponível.' }, 503);
    const body = await c.req
        .json()
        .catch(() => ({}));
    const genesisId = body.genesis_catalog_id?.trim();
    if (!genesisId)
        return c.json({ ok: false, error: 'genesis_catalog_id obrigatório.' }, 400);
    ...
    const { data: player, error: fetchErr } = await sb
        ...
        .maybeSingle();
    if (fetchErr || !player)
        return c.json({ ok: false, error: 'Jogador não encontrado.' }, 404);
    if (!player.listed_on_market)
        return c.json({ ok: false, error: 'Jogador não está à venda.' }, 409);
    const mintOverall = player.mint_overall;
