...
| **B — Beats** ✅ 2026-06-12 | `quickBeatDirector.ts` (efeito local determinístico das decisões via SpiritRng seedado, vereditos hit/neutral/miss, Leitura de Jogo n/m) + `AnalystBeatCard` (card amarelo, 3 escolhas, janela 5s, pesos ocultos) + `QuickPlanPlayer` reescrito como scheduler sequencial pausável com seam `onSecondHalf` (replan no 45') + feed ≤10 itens de alto sinal. Preview e2e em /match/quick-plan-preview. Narração = template Python local (GameSpirit/OpenAI fica pro polish). Teste: `npm run test:quick-beats`. APOSENTAR `quickInteractiveMoments.ts` fica pra Fase C (quando MatchQuick trocar de motor — mexer antes mudaria o modo em produção sem substituto). | `quickBeatDirector.ts`, `AnalystBeatCard.tsx`, `QuickPlanPlayer.tsx`, `QuickPlanPreview.tsx` |
| **C — Experiência completa** ✅ 2026-06-12 | `MatchQuickEngaged.tsx` gateado por flag no topo do `MatchQuick.tsx`. **Relógio corrido** 1→90 (não pula evento a evento); banner persistente AO VIVO (cinza=jogo / amarelo=lance); **clutch antes de cada gol** (Chutar/Driblar/Tocar · Cercar/Carrinho/Combate com contexto+feedback — `quickClutch.ts`); pênalti = escolha de batedor no feed (sem modal); lesão/vermelho = banner de substituição; **5 cards dos DOIS times** (3 melhores+2 piores, roster editorial) substituíveis a qualquer momento; intervalo com sub+formação+tática→replan; pós-jogo refeito com card de MVP Crown Jewel + stats + Leitura de Jogo; nomes em Moret amarela/branca no feed+leitura. Design 100% Legacy Tech (tokens, sem emoji, Moret/Agency/Inter). Decisões em 10'/30'/INTERVALO/60'/80'. Testes: `quick-matchup`, `quick-beats`, `quick-engaged`, `quick-clutch`. **Flag OFF até Fase D creditar progressão.** |
