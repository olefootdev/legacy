...
function adminPackEntries(situations) {
    ...
    try {
        ...
        if (p.v !== 2 || !Array.isArray(p.narrativePacks))
            return [];
        const out = [];
        for (const pack of p.narrativePacks) {
            if (!situations.includes(pack.bucket))
                continue;
            for (const line of pack.lines) {
                if (line.trim())
                    out.push({ situation: pack.bucket, template: line.trim(), tags: ['admin'], weight: 15 });
            }
        }
        return out;
    }
 ...
 */
export function pickLine(situation, params, seed) {
...
//# sourceMappingURL=narrationSeed.js.map