...
function pick(arr, eventType, seed, minute) {
...
export function shot(p) {
    ...
    ], 'shot', p.min * 17 + p.shooter.length, p.min);
}
export function shotBlock(p) {
    if (p.defender) {
        return pick([
        ...
        ], 'shotBlock', p.min + (p.defender.length ?? 0), p.min);
    }
    return pick([
    ...
    ], 'shotBlock', p.min, p.min);
}
export function shotSave(p) {
    if (p.keeper) {
        return pick([
...
export function progress(p) {
    if (p.receiver) {
        ...
        ], 'progress', p.min + p.carrier.length, p.min);
    }
    return pick([
...
export function recycle(p) {
    if (p.receiver) {
        ...
        ], 'recycle', p.min + p.passer.length, p.min);
    }
    return pick([
...
// ── Away genérico ───────────────────────────────────────
export function awayShotWide(p) {
    if (p.recoverer) {
        return pick([
...
export function awayBuild(p) {
    return `${s(p.min)} ${p.team} constrói jogada.`;
}
export function turnover(p) {
    return `${s(p.min)} ${p.team} retoma a posse.`;
}
export function noCarrierRecycle(p) {
    return `${s(p.min)} ${p.team} recua para reorganizar.`;
}
//# sourceMappingURL=narrativeTemplates.js.map