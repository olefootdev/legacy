...
import { PRE_GOAL_DURATION_MS } from '@/engine/types';
import { adjustHomeShotWeights, causalOutcomeFromHomeShot, createGoalOverlay, DANGEROUS_FOUL_PROB, DEFAULT_HOME_SHOT_WEIGHTS, initialPenaltyState, patchAfterAwayShotWide, patchAfterHomeShot, penaltyOverlayForStage, PENALTY_FROM_FOUL_PROB, rollHomeShotLogicalOutcome, rollGkSaveSubtype, rollTackleOutcome, } from './spiritStateMachine';
...
import { PlayerProgressionManager, SIGNATURE_MOVES } from '@/progression/playerProgression';
import { applyIntensityToShotChance, applyIntensityToDefense } from '@/match/quickTacticalIntensity';
function dist(a, b) {
    const dx = a.x - b.x;
    const dy = a.y - b.y;
    return Math.hypot(dx, dy);
}
function zoneFromBallX(x) {
    if (x < 38)
        return 'def';
    if (x < 68)
        return 'mid';
...
function spiritShotTargetUI(side, shooter) {
...
function nearestTeammateDistance(onBall, mates) {
    if (!onBall)
        return 50;
    let best = 1e9;
    for (const m of mates) {
        if (m.playerId === onBall.playerId)
            continue;
 ...
 */
function nearestHomeToBall(ball, homePlayers) {
    ...
    for (const p of homePlayers) {
        ...
        if (!best || d < best.dist)
            best = { player: p, dist: d };
    }
    return best;
}
function detectOutOfPlay(ball, carrier, homePlayers) {
    // Linha lateral (saiu pelo Y).
    if (ball.y <= 4 || ball.y >= 96) {
        const zone = ball.x < 38 ? 'def' : ball.x < 68 ? 'mid' : 'att';
        const awardedTo = carrier === 'home' ? 'away' : 'home';
        return {
            kind: 'throw_in',
            awardedTo,
            zone,
...
function densityNearBall(ball, mates, radius = 18) {
    let c = 0;
    for (const m of mates) {
        if (dist(ball, { x: m.x, y: m.y }) < radius)
            c += 1;
    }
    return c;
}
function countOpponentsWithin(ball, opps, radius = 8) {
    if (!opps)
        return 0;
    let c = 0;
    for (const o of opps)
        if (dist(ball, { x: o.x, y: o.y }) < radius)
            c += 1;
    return c;
}
function findFreeForwardTeammate(onBall, mates, opps, side) {
    if (!onBall || !mates)
        return null;
...
function pickAction(ctx) {
    ...
    const urgencyByContext = (scoreDiff < 0 && desperateTime) ? 0.35 : // Perdendo nos acréscimos → urgência máxima
    ...
    if (ctx.possession === 'away' && deepDefense && highPress) {
        if (!m)
            return 'press';
        if (Math.random() < Math.min(0.96, 0.88 * m.awayPressMult))
            return 'press';
    }
    if (ctx.possession === 'home' && ctx.ballZone === 'att' && (ctx.onBall?.role === 'attack' || ctx.onBall?.role === 'mid')) {
        if (isolated && ctx.crowdPressure.longPassStress > 1.05)
            return 'recycle';
        ...
        const shotBias = style.shootingProfile * 0.25 +
    ...
    if (ctx.possession === 'home' && style.buildUp > 0.72 && !freeFwd && urgencyByContext <= 0 && Math.random() < 0.22)
        return 'clear';
    if (ctx.possession === 'home' && style.verticality > 0.72 && freeFwd)
        return 'progress';
    if (ctx.possession === 'home' && style.verticality > 0.72 && Math.random() < 0.24)
        return 'progress';
    // Urgência: quando perdendo, reduz recycle (prefere avançar mesmo sob pressão moderada).
    if (ctx.possession === 'home' && style.verticality < 0.28 && !underPressure && urgencyByContext <= 0 && Math.random() < 0.28)
        return 'recycle';
...
function secondaryMate(ctx, excludeId) {
    const others = (ctx.homePlayers ?? []).filter((p) => p.playerId !== excludeId && p.name);
    if (others.length === 0)
        return undefined;
    if (!ctx.ball)
...
function narrativeFor(action, name, minute, _pressure, homeShort, _zone, ctx) {
...
export function buildSpiritContext(input) {
    ...
    const onBallKnowledge = input.possession === 'home' && input.onBall
...
function postGoalRestart(L, ball, nextPossession) {
    L.push({
        type: 'phase_change',
 ...
 */
function detectCounter(possessionAtTickStart, scorerSide, recentCausal) {
    if (possessionAtTickStart !== scorerSide)
        return true;
...
function commitGoal(input) {
    ...
    const nextPossession = scorerSide === 'home' ? 'away' : 'home';
...
export function gameSpiritTick(ctx, awayShort, causalSeqStart, nowMs = Date.now()) {
    ...
    const fairPlay = nearestDefender?.attributes?.fairPlay ?? 60;
    const aggression = nearestDefender?.attributes?.aggression ?? 50;
    ...
    if (nearestDefender) {
        ...
        if (defendRes.fired)
            dangerousFoulProbAdj *= (1 - defendRes.finalEffect);
    }
    if (ctx.possession === 'home' &&
        ...
        if (toPenalty) {
            L.push({
                type: 'phase_change',
                payload: { from: 'LIVE', to: 'PENALTY', reason: 'foul_in_box' },
            });
            ...
            return {
                ...
                spiritMeta: {
                    ...
                    spiritOverlay: penaltyOverlayForStage('banner', takerName, ctx.homeShort ?? 'Casa', awayShort, nowMs, 2000),
                },
            };
        }
        return {
    ...
    let next = ctx.possession;
    let ball = { ...ctx.ball };
    let goalFor;
    let goalScorerPlayerId;
    let goalBuildUp;
    let threatBar01;
    const cp = ctx.crowdPressure;
    const homeStat = ctx.possession === 'home' && ctx.onBall
        ? {
        ...
        if (action === 'shot') {
            ...
            L.push({
                ...
                payload: {
                    ...
                    ...(fromCorner ? { strike: 'header' } : fromFreeKick ? { strike: 'placed' } : {}),
                },
            });
            ...
            const sumW = Math.max(0.0001, weights.goal + weights.post_in + weights.save + weights.block + weights.wide + weights.post_out + weights.miss_far);
            lastShotPreview = {
                side: 'home',
                ts: nowMs,
                probs: {
    ...
    const skipBallValidator = spiritMeta?.spiritOverlay ||
